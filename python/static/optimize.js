...
M.addStrings({
    zh: { o_title: "系统优化与修复", o_run: "运行选中", o_needAdmin: "需管理员",
          ...
          o_done: (ok, f) => `完成：成功 ${ok}，失败/跳过 ${f}`, o_adminHint: "以管理员身份运行可解锁全部项" },
    en: { o_title: "System optimize & repair", o_run: "Run selected", o_needAdmin: "needs admin",
          ...
          o_done: (ok, f) => `Done: ${ok} ok, ${f} failed/skipped`, o_adminHint: "Run as Administrator to unlock all actions" },
    ja: { o_title: "システム最適化と修復", o_run: "選択を実行", o_needAdmin: "管理者必要",
          ...
          o_done: (ok, f) => `完了：成功 ${ok}、失敗/スキップ ${f}`, o_adminHint: "管理者として実行すると全項目が有効" },
});
...
function render(el) {
    el.innerHTML = `
      ...
      ${isAdmin ? "" : `<p class="hint">${M.t("o_adminHint")}</p>`}
